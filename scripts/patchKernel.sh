...
patch -p1 < ${LIBREALSENSE_DIR}/scripts/realsense-camera-formats-${ubuntu_codename}-${kernel_branch}.patch 
echo -e "\e[32mApplying realsense-metadata patch\e[0m"
patch -p1 < ${LIBREALSENSE_DIR}/scripts/realsense-metadata-${ubuntu_codename}-${kernel_branch}.patch
echo -e "\e[32mApplying realsense-hid patch\e[0m"
patch -p1 < ${LIBREALSENSE_DIR}/scripts/realsense-hid-${ubuntu_codename}-${kernel_branch}.patch
